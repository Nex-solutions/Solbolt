...
const real_funds_demo_1 = require("./real-funds-demo");
...
program
    ...
    const demo = new demo_runner_1.DemoRunner({
...
program
    ...
    console.log(chalk_1.default.yellow('⚠️  WARNING: This will use real SOL from your wallet!\n'));
    const demo = new real_funds_demo_1.RealFundsDemo({
        network: options.network,
        transactionCount: parseInt(options.transactions),
...
program
    ...
    .action(async (options) => {
    const manager = new channel_manager_1.ChannelManager();
...
program.on('command:*', () => {
    console.error(chalk_1.default.red('Error: Unknown command'));
    program.help();
});
// Parse arguments
program.parse();
